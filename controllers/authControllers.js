  ...
  try {
    await Employe.findById(req.params.EmployeeId).then(async (employed) => {
      if (!employed.firstTimeOnboard) {
        ...
        (employed.password = hashpassword), (employed.opsCode = hashops);
...
module.exports.paymentRevoke_patch = async (req, res) => {
  try {
    await CreditCustomerPayment.findById(req.params.paymentId).then(
      async (paymentlog) => {
        if (paymentlog.status === "Reversed") {
          throw new Error("This payment has already been Reversed");
        } else if (
          paymentlog.status === "posted" &&
          paymentlog.collectionRef === "PMNT"
        ) {
          await Account.findOne({ bankCode: paymentlog.bankAccount })
            ...
            .then(async () => {
              const customer = await Customer.findById(
                ObjectId(paymentlog.BillNo)
              );
              customer.Debt + paymentlog.crAmount;
              await Customer.updateOne(
                { _id: ObjectId(paymentlog.BillNo) },
                { $set: { Debt: customer.Debt + paymentlog.crAmount } }
              );
              //generate report for customer
              await CustomerReport.create({
...
module.exports.Report_get = async (req, res, next) => {
  ...
  const Bestproduct = await Product.find().then((product) => {
    topSeller = product.sort(function (a, b) {
  ...
  const TodaySale = BillTotal.filter((b) => {
    return (
      b.startDate === `${d.getFullYear()}-${d.getMonth() + 1}-${d.getDate()}` &&
      b.billStatus.toLowerCase() !== "CANCELLED".toLowerCase()
    );
  })
    .map((bill) => {
  ...
  //  top deptor
  const debtor = await Customer.find().then((debtor) => {
    topSeller = debtor
      .sort(function (a, b) {
        return b.Debt - a.Debt;
      })
      .filter((debtor) => {
        return debtor.Debt > 0;
      });
    return topSeller[0];
  });
  const WH = await WHouse.find();
  ...
  try {
    await Vendor.findById(req.body.Vendor).then(async (vendor) => {
      await Vendor.updateOne(
        ...
        {
          $set: {
            Balance: parseInt(vendor.Balance) - parseInt(req.body.INCENTIVE),
          },
        }
      ).then(async (acknowledged) => {
        if (acknowledged) {
  ...
  if (vendorBill.StoreKeeperConfirmation === false) {
    try {
      if (req.body.status === "Approved") {
        ...
        res.status(500).json({
          deleteMessage: `Response Acknowledged. Invoicer will be Notified to Check for ${req.body.RejectionReason}. Please Note that the LPO will be deleted `,
        });
...
module.exports.productsJson_get = async (req, res) => {
  const productsJson = await Product.find({
    Vendor: ObjectId(req.params.vendorID),
  });
...
module.exports.registerCustomerPayment_post = async (req, res) => {
  try {
    const customer = await Customer.findById(req.body.customer);
    if (
      req.body.TRANSACTION === "CR" &&
      customer.category === "Credit-Customer"
    ) {
      ...
      await CreditCustomerPayment.create({
        ...
        if (payment) {
          ...
          await CustomerReport.create({
            ...
            if (report) {
              await Customer.updateOne(
                { _id: ObjectId(req.body.customer) },
                { $set: { Debt: NewDebt } }
              );
              await Account.updateOne(
                { _id: account._id },
          ...
          await Customer.findById(ObjectId(req.body.customer)).then(
            async (debtor) => {
              if (NewDebt <= 0) {
                await Customer.updateOne(
                  { _id: ObjectId(debtor._id) },
                  { $set: { lastPayDate: "" } }
                );
              }
            }
          ...
          res.status(200).json({
            message: `Payment of ₦${req.body.Amount} Registered for ${customer.Username} successfully`,
          });
        } else {
